...
import type { EmailMessage, WhatsAppMessage } from '../ports/index.js'
...
export function formatWhatsAppMessage(payload: AnamnesisPayload): WhatsAppMessage {
  ...
  const patientName = truncateParam(
    payload.patient.socialName.trim() || payload.patient.name,
  )
  const patientPhone = truncateParam(payload.patient.phone || '—')
  const complaint = truncateParam(payload.mainComplaint || 'Não informada')

  return {
    text,
    templateBodyParameters: [patientName, patientPhone, complaint],
  }
}

function truncateParam(value: string, max = 200): string {
